...
ES.onmessage = message => {
	...
	$('.dogs-res').css('width', `${dogsVotesPersent}%`).attr('aria-valuenow', dogsVotesPersent).text(`Собаки - ${dogsVotesPersent}%`);

	$('.cats-res').css('width', `${catsVotesPersent}%`).attr('aria-valuenow', catsVotesPersent).text(`Кошки - ${catsVotesPersent}%`);

	$('.parrots-res').css('width', `${parrotsVotesPersent}%`).attr('aria-valuenow', parrotsVotesPersent).text(`Попугаи - ${parrotsVotesPersent}%`);
}
